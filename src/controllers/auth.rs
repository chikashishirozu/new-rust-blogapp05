use crate::{
    ...
    models::{
        _entities::user::{self, Column, Entity, Model},
...
async fn register(
    ...
    let user = user::Model::find_by_email(&ctx.db, &params.email).await?;
...
async fn verify(State(ctx): State<AppContext>, Path(token): Path<String>) -> Result<Response> {
    let user = match user::Model::find_by_verification_token(&ctx.db, &token).await {
        Ok(Some(user)) => user,
    ...
    if user.email_verified_at.is_some() {
        tracing::info!(pid = user.pid.as_ref().map(|p| p.to_string()).unwrap_or_default(), "user already verified");
    } else {
        let active_model = user.into_active_model();
        let user = active_model.verified(&ctx.db).await?;
        tracing::info!(pid = user.pid.as_ref().map(|p| p.to_string()).unwrap_or_default(), "user verified");
    }
...
async fn login(State(ctx): State<AppContext>, Json(params): Json<LoginParams>) -> Result<Response> {
    let user = match user::Model::find_by_email(&ctx.db, &params.email).await {
        Ok(Some(user)) => user,
        Ok(None) => {
            tracing::debug!(
                email = params.email,
    ...
    let token = user
        .generate_jwt(&jwt_secret.secret, jwt_secret.expiration as i64)
...
async fn magic_link(
    ...
    let active_model = user.into_active_model().create_magic_link(&ctx.db).await?;
...
async fn magic_link_verify(
    ...
    let active_model = user.into_active_model().clear_magic_link(&ctx.db).await?;

    let jwt_secret = ctx.config.get_jwt_config()?;

    let token = user
        .generate_jwt(&jwt_secret.secret, jwt_secret.expiration.try_into().unwrap())
        .or_else(|_| unauthorized("unauthorized!"))?;
...
async fn resend_verification_email(
    ...
    if user.email_verified_at.is_some() {
        tracing::info!(
            pid = user.pid.as_ref().map(|p| p.to_string()).unwrap_or_default(),
    ...
    AuthMailer::send_welcome(&ctx, &user).await?;
    tracing::info!(pid = user.id.to_string(), "Verification email re-sent");
