...
impl AuthMailer {
    ...
    pub async fn send_welcome(ctx: &AppContext, user: &user::Model) -> Result<()> {
        Self::mail_template(
            ctx,
            &welcome,
            mailer::Args {
    ...
    pub async fn forgot_password(ctx: &AppContext, user: &user::Model) -> Result<()> {
        Self::mail_template(
            ...
            mailer::Args {
                to: user.email.clone(),
                locals: json!({
                  "name": user.name.as_ref().unwrap_or(&"User".to_string()),
                  "resetToken": user.reset_token.as_ref().ok_or_else(|| Error::string("No reset token"))?,
                  "domain": ctx.config.server.full_url()
                }),
                ..Default::default()
            },
    ...
    pub async fn send_magic_link(ctx: &AppContext, user: &user::Model) -> Result<()> {
        Self::mail_template(
            ...
            mailer::Args {
                ...
                locals: json!({
                  "name": user.name.as_ref().unwrap_or(&"User".to_string()),
                  "token": user.magic_link_token.clone().ok_or_else(|| Error::string("No magic link token"))?,
