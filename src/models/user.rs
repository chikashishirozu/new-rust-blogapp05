...
use crate::models::_entities::user::{Entity as UserEntity, Model as UserModel, ActiveModel as UserActiveModel};

use crate::models::_entities::post::Model as PostModel;
...
impl Model {
    ...
    pub async fn find_by_verification_token(
        ...
        let user = user::Entity::find()
            .filter(
                model::query::condition()
                    .eq(user::Column::EmailVerificationToken, token)
                    .build(),
            )
            .one(db)
            .await?;
    ...
    pub async fn find_by_magic_token(db: &DatabaseConnection, token: &str) -> ModelResult<Self> {
        ...
        if let Some(expired_at) = user.magic_link_expiration {
            if expired_at >= Local::now() {
                ...
                tracing::debug!(
                    user.pid.as_ref().map(|p| p.to_string()).unwrap_or_default(),
    ...
    pub async fn create_with_password(
        ...
        if user::Entity::find()
            .filter(
                model::query::condition()
                    .eq(user::Column::Email, &params.email)
                    .build(),
            )
            .one(&txn)
            .await?
...
// impl ActiveModelBehavior for user::ActiveModel {}  // パスを修正

// ファイル: src/models/user.rs の末尾
...
mod tests {
    ...
    async fn test_create_user() {
        ...
        let result = ActiveModel::create_with_password(&db, &params).await;
        assert!(result.is_ok());
    ...
    async fn test_find_by_email() {
        ...
        let params = RegisterParams {
            ...
            password: "password123".to_string(),
            name: "Find User".to_string(),
        };
        ActiveModel::create_with_password(&db, &params).await.unwrap();

        // 検索テスト
        let result = Model::find_by_email(&db, "find@example.com").await;
        assert!(result.is_ok());
    }
    ...
    async fn test_verify_password() {
        ...
        let user = ActiveModel::create_with_password(&db, &params).await.unwrap();

        // 正しいパスワード
        let valid = user.verify_password("secret123").await.unwrap();
        assert!(valid);

        // 間違ったパスワード
        let invalid = user.verify_password("wrong").await.unwrap();
        assert!(!invalid);
    }
}
